    ...
    <div className="space-y-4">
        ...
        />
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button className="w-full sm:w-auto text-xs md:text-sm">Create New Loan</Button>
            </DialogTrigger>
            <DialogContent className="w-[95vw] sm:w-[90vw] md:w-[85vw] lg:w-[80vw] max-w-7xl">
              <DialogHeader>
                <DialogTitle className="text-lg md:text-xl">Create New Loan</DialogTitle>
              </DialogHeader>
              <CreateLoan onSuccess={() => {
      ...
      <div className="rounded-md border overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead onClick={() => handleSort('fullName')} className="cursor-pointer text-xs md:text-sm whitespace-nowrap">
                Full Name {sortField === 'fullName' && (sortDirection === 'asc' ? '↑' : '↓')}
              </TableHead>
              <TableHead onClick={() => handleSort('requestedAmount')} className="cursor-pointer text-xs md:text-sm whitespace-nowrap">
                Requested {sortField === 'requestedAmount' && (sortDirection === 'asc' ? '↑' : '↓')}
              </TableHead>
              <TableHead onClick={() => handleSort('totalToPay')} className="cursor-pointer text-xs md:text-sm whitespace-nowrap">
                Total {sortField === 'totalToPay' && (sortDirection === 'asc' ? '↑' : '↓')}
              </TableHead>
              <TableHead onClick={() => handleSort('dailyPayment')} className="cursor-pointer text-xs md:text-sm whitespace-nowrap">
                Daily {sortField === 'dailyPayment' && (sortDirection === 'asc' ? '↑' : '↓')}
              </TableHead>
              <TableHead onClick={() => handleSort('paidLoan')} className="cursor-pointer text-xs md:text-sm whitespace-nowrap">
                Paid {sortField === 'paidLoan' && (sortDirection === 'asc' ? '↑' : '↓')}
              </TableHead>
              <TableHead onClick={() => handleSort('startDate')} className="cursor-pointer text-xs md:text-sm whitespace-nowrap">
                Start {sortField === 'startDate' && (sortDirection === 'asc' ? '↑' : '↓')}
              </TableHead>
              <TableHead onClick={() => handleSort('expectDate')} className="cursor-pointer text-xs md:text-sm whitespace-nowrap">
                Expected {sortField === 'expectDate' && (sortDirection === 'asc' ? '↑' : '↓')}
              </TableHead>
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={11} className="text-center text-xs md:text-sm">Loading...</TableCell>
              </TableRow>
            ) : paginatedLoans.length === 0 ? (
              <TableRow>
                <TableCell colSpan={11} className="text-center text-xs md:text-sm">No loans found</TableCell>
              </TableRow>
            ) : (
      ...
      {!loading && filteredAndSortedLoans.length > 0 && (
        <div className="flex items-center justify-between px-2">
          <div className="text-sm text-muted-foreground">
            Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredAndSortedLoans.length)} of {filteredAndSortedLoans.length} entries
          </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                <div>
                  ...
                  <p className="font-mono text-xs md:text-sm">{selectedLoan.loanId}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-xs md:text-sm">{formatDate(selectedLoan.startDate)}</p>
                </div>
                <div>
                  <Label className="text-xs md:text-sm">Expected Date</Label>
                  <p className="text-xs md:text-sm">{formatDate(selectedLoan.expectDate)}</p>
                </div>
                <div>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-xs md:text-sm">Paid By</TableHead>
        ...
        <DialogContent className="max-w-lg print:block">
          ...
          {(printLoan && printUser) ? (
            <div id="print-section" className="space-y-2">
              <div><b>User Full Name:</b> {printUser.fullName}</div>
              <div><b>Username:</b> {printUser.username}</div>
              <div><b>Phone:</b> {printUser.phoneNumber}</div>
          ...
          <div className="flex gap-2 mt-4">
            <Button onClick={() => {
              ...
              if (printWindow && printContents) {
                printWindow.document.write('<html><head><title>Print Loan & User Info</title>')
